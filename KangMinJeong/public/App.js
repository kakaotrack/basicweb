...
class UFOLaser extends GameObject {
	constructor(x, y) {
	  ...
	  let id = setInterval(() => {
		if (!this.dead) {
		  this.y = this.y < HEIGHT ? this.y + 20 : this.y; // 아래쪽으로 이동하도록 변경
		  if (this.y >= HEIGHT) {
			this.dead = true;
		  }
		} else {
...
class Monster extends GameObject {
	constructor(x, y) {
		...
		this.direction = Math.random() < 0.5 ? 1 : -1;
		    ...
		    // 랜덤으로 초기 내려오는 속도 설정
			this.speed = {
				x: 0, // 가로 방향 속도는 0으로 설정 (수평 이동하지 않음)
				y: Math.random() * 50 + 20, // 세로 방향 속도를 1에서 6까지의 랜덤 값으로 설정
			  };
	
		let id = setInterval(() => {
		  if (!this.dead) {
			this.x = this.x + this.direction * 30;
			if (this.x <= 0 || this.x >= WIDTH - this.width) {
			  this.direction *= -1;
			}
			this.y = this.y < HEIGHT ? this.y + this.speed.y : this.y;
			this.randomAttackPattern();
...
class Game {
	constructor() {
	  ...
	  eventEmitter.on(Messages.UFO_FIRE, () => {
		if (coolDown === 0) {
		  let l = new UFOLaser(ufo.x - 45, ufo.y + 30);
		  l.img = laserGreenImg;
		  gameObjects.push(l);
		  cooling();}
...
function createMonsters(monsterImg, count) {
	for (let i = 0; i < count; i++) {
	  setTimeout(() => {
		const x = Math.floor(Math.random() * (WIDTH - 98)); // 랜덤한 x 위치
		const y = 0; // 적이 생성되면서 위에서부터 내려오기 시작
  
		const monster = new Monster(x, y);
		gameObjects.push(monster);
		monster.img = monsterImg;
	  }, Math.random() ); // 적이 생성되기까지의 랜덤한 지연 시간 설정
	}
  }

  function createUFOs(ufoImg, count) {
	for (let i = 0; i < count; i++) {
	  setTimeout(() => {
		const x = Math.floor(Math.random() * (WIDTH - 98)); // 랜덤한 x 위치
		const y = 0; // 적이 생성되면서 위에서부터 내려오기 시작
  
		const ufo = new UFO(x, y);
		ufo.img = ufoImg;
		ufo.shootLaser(); // UFO가 생성될 때 레이저를 발사하도록 하는 부분입니다
...
function checkGameState(gameLoopId) {
	...
	const ufo = gameObjects.filter((go) => go.type === 'ufo');
	...
	const ufoLasers = gameObjects.filter((go) => go.type === 'UFOLaser');
   	...
   	ufoLasers.forEach((ufoLaser) => {
		if (intersectRect(ufoLaser.rectFromGameObject(), hero.rectFromGameObject())) {
			eventEmitter.emit(Messages.COLLISION_MONSTER_HERO, { ufoLaser, id: gameLoopId });
		}
	});
...
function runGame() {
	// 게임 시작 시에만 gameObjects 배열 초기화
	if (!game.end) {
	  gameObjects = [];
	  game.life = 3;
	  game.points = 0;
	  game.end = false;
	  //createMonsters(monsterImg, 20);
	  createHero(heroImg);
	  continuouslyCreateUFOs();
	}
...
window.onload = async () => {
  ...
  if (canvas) {
	...
	createHero(heroImg);
	function continuouslyCreateUFOs() {
		if (!hero.dead) {
			createUFOs(ufoImg, 1);
			setTimeout(continuouslyCreateUFOs, 5000); // 5초 간격으로 실행
		}
		}

	function continuouslyCreateMonsters() {
		if (!hero.dead) {
		  createMonsters(monsterImg, 1);
		  setTimeout(continuouslyCreateMonsters, 2000); // 5초 간격으로 실행

		}
	  }
	  continuouslyCreateUFOs();
	  continuouslyCreateMonsters();
