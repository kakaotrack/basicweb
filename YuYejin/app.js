...
class Monster extends GameObject {
	constructor(x, y) {
		...
		this.width = 60;
		this.height = 60;
		this.moveInterval = 2000; // 2초마다 이동
		this.move();
	}
	...
	move() {
		setInterval(() => {
			if (!this.dead) {
				const moveX = this.getRandomMove(); // x-방향으로 랜덤 이동 (-1, 0, 1)
				const moveY = 1; // 항상 아래로 이동

				this.x = Math.max(0, Math.min(WIDTH - this.width, this.x + moveX * 30));
				this.y = Math.max(0, Math.min(HEIGHT - this.height, this.y + moveY * 30)); // 항상 아래로 이동

				if (this.y >= HEIGHT - this.height) {
					this.dead = true;
					eventEmitter.emit(Messages.MONSTER_OUT_OF_BOUNDS);
				}
			}
		}, this.moveInterval);
	}
...
class Meteor extends GameObject {
	constructor() {
		const initialX = Math.random() * (WIDTH - 50); // 50은 meteorSmall의 가로 크기에 따라 조절
		super(initialX, 0);
		this.type = 'Meteor';
		this.width = 50; // meteorSmall의 가로 크기
		this.height = 50; // meteorSmall의 세로 크기
		this.moveInterval = 100; // 떨어지는 간격 (밀리초)
		this.move();
	}
...
class Game {
	constructor() {
		...
		eventEmitter.on(Messages.GAME_END_LOSS, (_, gameLoopId) => {
			game.end = true;
			displayMessage('Game Over 재시작하려면 Enter키를 누르세요', 'pink');
			clearInterval(gameLoopId);
		});

		eventEmitter.on(Messages.GAME_END_WIN, (_, gameLoopId) => {
			game.end = true;
			displayMessage('당신 덕분에 행성을 구했어요! 재시작하려면 Enter', 'pink');
			clearInterval(gameLoopId);
		});
		...
		eventEmitter.on(Messages.COLLISION_METEOR_HERO, () => {
			game.life--;
			if (game.life === 0) {
				hero.dead = true;
...
function displayLife() {
	...
	const START_X = canvas.width - 150 - 100;
	for (let i = 0; i < game.life; i++) {
		ctx.drawImage(lifeImg, START_X + (i + 1) * 45, canvas.height - 120);
	}
}
...
function createMonsters(monsterImg) {
	...
	const MONSTER_WIDTH = MONSTER_TOTAL * 60;
	const START_X = (canvas.width - MONSTER_WIDTH) / 2;
	const STOP_X = START_X + MONSTER_WIDTH;

	for (let x = START_X; x < STOP_X; x += 60) {
...
window.onload = async () => {
	...
	if(canvas){
		...
		ctx.fillStyle = 'black';
		ctx.fillRect(0, 0, canvas.width, canvas.height);
		displayMessage('Enter를 눌러 고양이 행성 지키기를 도와주세요!', 'pink');
